...
model = dict(
    ...
    backbone=dict(
        ...
        cfg=[["mv2", 32, 4, 2],
             ...
             ["former", 128, 9, 2],
             ["se", 256, 4, 1.5]],
        pretrained="checkpoint/tinynext_m_backbone.pth",
        out_indices=(3, 4),
        frozen_stages=-1,
        norm_eval=True,
        sync_bn=True),
    ...
    bbox_head=dict(
        type='SSDHead',
        in_channels=(128, 256, 512, 256, 256, 128),
        num_classes=80,
        use_depthwise=True,
        norm_cfg=dict(type='BN', eps=0.001, momentum=0.03),
        act_cfg=dict(type='ReLU6'),
...
val_dataloader = dict(batch_size=64 * bs_ratio, num_workers=12 * min(bs_ratio, 2), dataset=dict(pipeline=test_pipeline))
...
optim_wrapper = dict(
    type='OptimWrapper',
    optimizer=dict(type='AdamW', lr=4e-3, betas=(0.9, 0.999), weight_decay=0.05,),
)
